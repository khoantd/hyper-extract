...
Hyper-Extract 支持多种接入方式：**OpenAI**、**阿里云百炼**、**本地 vLLM**、**远程 LiteLLM 代理**。统一使用 `create_client()` 接口，仅需修改第一行。
...
| **LiteLLM 代理** | 代理上配置的任意模型 | ✅ | ✅ | OpenAI 兼容网关；LLM 与嵌入共用同一 URL |
